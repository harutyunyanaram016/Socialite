		...
		@endif
			...
			<div class="privacy-question">
				...
				<fieldset class="form-group required {{ $errors->has('description') ? ' has-error' : '' }}">
					...
					@if($mode == "create")
						{{ Form::textarea('description', NULL , array('class' => 'form-control', 'placeholder' => trans('messages.create_page_placeholder') )) }}
					@else
						{{ Form::textarea('description', $staticPage->description , array('class' => 'form-control', 'placeholder' => trans('messages.create_page_placeholder') )) }}
					@endif	

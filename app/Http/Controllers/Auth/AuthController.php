...
//    public function addAffiliate($users){
//        $bool = false;
...
//                $referrals = User::where('affiliate_id', $user['id'])->where('id', '!=', $user['id'])->paginate(Setting::get('items_page', 10));
//
//                    foreach ($referrals as $userChild) {
//                        $referrals = User::where('affiliate_id', $userChild['id'])->where('id', '!=', $userChild['id'])->paginate(Setting::get('items_page', 10));
//                        return addAffiliate($referrals);
//                    }
//
//            }
//        }
...
//        }
        ...
        if ($request->affiliate) {
            $parent = Timeline::where('username', $request->affiliate)->first();
            $parent1 = DB::table('users')->join('timelines', 'timelines.id','=','users.timeline_id')->where('users.id', $parent->user->id)->first();
...
//                  });
                ...
                if($affiliate_id){
                    $parent = Timeline::where('username', $request->affiliate)->first();
                    $gamer = Gamer::where('user_id', $affiliate_id)->first();
                    $user_gamer = [
                        'user_id'             => $user->id,
                        'level'          => $gamer['level'] + 1,
                        'room'       => $gamer['room']
                    ];
                    $gamer_us = DB::table('gamers')->insert($user_gamer);
                }
    ...
    public function facebook()
    {
        ...
        if (!$user->id) {
            ...
            $avatar = Image::make($facebook_user->avatar_original);
  ...
  public function twitter()
  {
      ...
      $user = User::firstOrNew(['email' => $twitter_user->nickname.'@twitter.com']);
      if (!$user->id) {
          $request = new Request(['username'   => $twitter_user->nickname,
              'name'                           => $twitter_user->name,
              'email'                          => $twitter_user->nickname.'@twitter.com',
              'password'                       => bcrypt(str_random(8)),
              'gender'                         => 'none',
            ]);
          $timeline = $this->registerUser($request, true);

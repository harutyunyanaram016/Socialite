...
//    public function getReferrals($id, $users){
//        $referrals = User::whereIn('affiliate_id', $id)->where('id', '!=', Auth::user()->id)->paginate(Setting::get('items_page', 10));
//        if(!empty($referrals)){
//            $id = array();
//            echo 'sadasd ';
...
////            $users[] = $referrals;
////            var_dump($users);die;
//            return $this->getReferrals($id, $users);
//        }else{
//
//             return $users;
//        }
//    }
    ...
    public function ref($user, $i=0)
    {

        $user->childs =  DB::table('users')->join('timelines', 'timelines.id','=','users.timeline_id')->where('affiliate_id', $user->id)->get();
        if(!$user->childs || $i>2){
            return $user;
        }else{
            ++$i;
            foreach ($user->childs as $val){
                $this->ref($val);
            }
        }
    ...
    public function affliates($username)
    {
        ...
        $user = DB::table('users')->join('timelines', 'timelines.id','=','users.timeline_id')->where('users.id', Auth::user()->id)->first();
        $this->ref($user,0);
//        echo "<pre>";
//        print_r($user);die;
        $referrals = $user->childs;
//
//        $array = $this->ref();
...
////            var_dump($val['id']);
////        }die;
////        $parrnt = User::where('id', Auth::user()->affiliate_id)->paginate(Setting::get('items_page', 10));
//        foreach ($referrals as $key=>$val){
//            $id[] = $val['id'];
//        }
//
////        array_push($referrals, $parrnt);
//        echo "<pre>";
//        print_r($id);die;
//        foreach ($referrals1 as $key=>$val){
//            var_dump($key);die;
////            $referrals[$key]['referrals']  = $parrnt = User::where('affiliate_id', Auth::user()->affiliate_id)->where('id', '!=', Auth::user()->id)->paginate(Setting::get('items_page', 10));
//        }
//
        $theme = Theme::uses(Setting::get('current_theme', 'default'))->layout('default');
//
    ...
    public function getUseraffliates($user){
        $referrals = User::where('affiliate_id', $user['id'])->where('id', '!=', $user->id)->paginate(Setting::get('items_page', 10));
        if($referrals){
    ...
    public function joinedGroups($username)
    {
        ...
        $joined_groups_count = $user->groups()->get()->count();
        $following_count = $user->following()->where('status', '=', 'approved')->get()->count();
        $followers_count = $user->followers()->where('status', '=', 'approved')->get()->count();
        $followRequests = $user->followers()->where('status', '=', 'pending')->get();
        $follow_user_status = '';
